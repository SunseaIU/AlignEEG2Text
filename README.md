...
**AlignEEG2Text**: A language-agnostic two-stage framework for decoding natural
text from EEG signals — (1) EEG↔text **contrastive alignment** of a lightweight
...
*"Contrastive Alignment and LoRA-Adapted BART for Cross-Subject EEG-to-Text Decoding"*.
...
EEG (128 ch × T samples)
   │
   ▼
NICE EEG Encoder ── temporal Conv2D block (kernel 25) + spatial attention
   │                (NO graph attention / NO electrode-coordinate prior)
   ▼  256-d embedding
Stage 1: bidirectional InfoNCE contrastive alignment with precomputed
         BERT text embeddings of display lines (curriculum schedule, τ=0.07)
   │  → checkpoints/{subject}_contrastive_final.pt
   ▼
Joint Linear projection (Linear+ReLU+Dropout → Linear+Tanh → 16 × 768 prefix tokens)
   │
   ▼
...
`config_gpu.yaml`, Table I) and **25 for the controlled ablations**
(Fig. 3 / Table IV runner). Effective batch size is 128 via gradient accumulation
...
### Table IV — fine-tuning-only baseline (no Stage-1 contrastive pre-training)
...
### Audit retest of archived Stage-2 checkpoints (Table I verification)
...
The released per-fold JSONs reproduce every per-subject cell of Table I at
